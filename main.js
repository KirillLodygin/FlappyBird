...
let GameInitialize = function GameInitialize() {
    ...
    let DEBUG_MODE = false,
        ...
        CANVAS_WIDTH = window.innerWidth || document.documentElement.clientWidth ||
            document.getElementsByTagName('body')[0].clientWidth,
        CANVAS_HEIGHT = window.innerHeight || document.documentElement.clientHeight ||
    ...
    let Background,
        ...
        TitleText, InstructionsText, ScoreText, HighScoreTitleText,
        HighScoreText, PostScoreText, LoadingText,
        PostScoreClickArea,
        isScorePosted = false,
        isSoundEnabled = true,
        Leaderboard;

    let Game = new Phaser.Game(CANVAS_WIDTH, CANVAS_HEIGHT, Phaser.CANVAS, SCENE);

    let gameScore = 0;

    let BootGameState = new Phaser.State();
    ...
    let PreloaderGameState = new Phaser.State();
    ...
    let toggleSound = function toggleSound() {
        if (isSoundEnabled) {
    ...
    let createBackground = function createBackground() {
    ...
    let createClouds = function createClouds() {
        function makeNewCloud(cloudX, startTimer) {
            ...
            let cloudY = Math.random() * Game.world.height / 2,
    ...
    let createTexts = function createTexts() {
        ...
        PostScoreClickArea = new Phaser.Rectangle(PostScoreText.x - PostScoreText.width * 5, PostScoreText.y -
            PostScoreText.height, PostScoreText.width + 200, PostScoreText.height * 4);
    };

    let createSounds = function createSounds() {
    ...
    let createBird = function createBird() {
    ...
    let birdFlap = function birdFlap() {
    ...
    let createPipes = function createPipes(timer) {
        if (timer) {
    ...
    function calcDifficult() {
        return AVAILABLE_SPACE_BETWEEN_PIPES + (Math.floor(Math.random() * AVAILABLE_SPACE_BETWEEN_PIPES)) *
            ((gameScore > MAX_DIFFICULT ? MAX_DIFFICULT : MAX_DIFFICULT - gameScore) / (MAX_DIFFICULT + 1));
    }

    let getModifiedSpeed = function getModifiedSpeed() {
        return GAME_SPEED + gameScore * 5;
    };

    function makeNewPipe(pipeY, isFlipped) {
        let pipe = Pipes.create(Game.world.width, pipeY + (isFlipped ? -calcDifficult() : calcDifficult()) / 2, 'pipe');
    ...
    function makePipes() {
        let pipeY = ((Game.world.height - 16 - calcDifficult() / 2) / 2) + (Math.random() > 0.5 ? -1 : 1) *
        ...
        let newTime = Game.rnd.integerInRange(PIPE_SPAWN_MIN_INTERVAL, PIPE_SPAWN_MAX_INTERVAL) -
            getModifiedSpeed() * 2;
    ...
    let addScore = function addScore(_, spaceInPipe) {
    ...
    let getScore = function getScore() {
        if (Leaderboard) {
            Leaderboard.fetch({
                sort: 'desc',
                best: true,
    ...
    let postScore = function postScore() {
        if (Leaderboard) {
            Leaderboard.post({
                score: gameScore
            }, function() {
    ...
    let HighScoreStateClick = function HighScoreStateClick() {
        if (Game.state.current == 'GameOver' && Phaser.Rectangle.contains(PostScoreClickArea,
                Game.input.x, Game.input.y) && !isScorePosted) {
            postScore();
            PostScoreText.setText("");
            isScorePosted = true;
        } else {
